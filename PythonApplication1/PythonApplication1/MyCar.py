...
class MyCar:
    ...
    def __init__(self, brand, model, year, engine, cost, vin):
        self.brand, self.model, self.year, self.engine, self.cost, self.vin =\
           brand, model, year, engine, cost, vin
    ...
    def __fuzzy_compare(self, query):
        ...
        score = 0
        for m_words, method in zip([BRAND_WORDS, MODEL_WORDS, YEAR_WORDS, ENGINE_WORDS, ENGINE_WORDS, COST_WORDS],
                                   [by_brand, by_model, by_year, by_en_type, by_en_pow, by_cost]):
            if m_words & q_words:
